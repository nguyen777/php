<?php 
    ...
    if(!empty($_POST)) {
        ...
        if(isset($_POST['update'])) {
            if($MaSV !='' && $FullName != ''){
                $sql = "UPDATE SinhVien SET HoTen='$FullName',NgaySinh = '$Birtday',MaKhoa ='$Khoa' WHERE MaSV = '$MaSV' ";
                $result = $conn->query($sql);
            }
        }
...
<body>
    ...
    <form  method="POST">
        ...
        <td>
        <input type="submit" onclick="return confirm('Bạn có chắc chắn xóa sinh viên này?')" name="delete" value="delete">
        </td>
